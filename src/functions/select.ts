...
import { z } from "zod"
import { DeviceOrAlias, RegisterOrAlias, StringOrNumber } from "../ZodTypes"
...
select.seq = (env, data) => {
	const [op1, op2, op3] = z.tuple([RegisterOrAlias, StringOrNumber, StringOrNumber]).parse(data)
...
select.sap = (env, data) => {
	const [op1, op2, op3, op4] = z.tuple([RegisterOrAlias, StringOrNumber, StringOrNumber, StringOrNumber]).parse(data)
...
select.sdse = (env, data) => {
	const [op1, op2] = z.tuple([RegisterOrAlias, DeviceOrAlias]).parse(data)
...
select.snan = (env, data) => {
	const [op1, op2] = z.tuple([RegisterOrAlias, RegisterOrAlias]).parse(data)
