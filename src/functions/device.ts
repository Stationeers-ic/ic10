...
export const device: { [key: string]: icFunction } = {
    ...
    lb: (env, data) => {
        const [op1, op2, op3, op4] = z.tuple([z.string(), z.string(), z.string(), z.string()]).parse(data)
        env.set(op1, env.get(`${env.getAlias(op2)}.${env.getAlias(op3)}.${env.getAlias(op4)}`))
    },
    sb: (env, data) => {
        const [op1, op2, op3] = z.tuple([z.string(), z.string(), z.string().or(z.number())]).parse(data)
        env.batchWrite(env.get(op1), op2, env.get(op3))
    },
}