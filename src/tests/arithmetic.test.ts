...
import {run, runFunc} from "./testUtils";
...
describe("arithmetic", () => {
    ...
    test("mod", () => {
        ...
        expect(runFunc(functions.mod, ['r0', 4, -2], {'r0': 4})).resolves.toBe(0);
        expect(runFunc(functions.mod, ['r0', 'r0', 'r0'], {'r0': -4})).resolves.toBe(0);
        expect(runFunc(functions.mod, ['r0', -4, 0])).rejects.toThrow();
        expect(runFunc(functions.mod, ['r0', -4, -0])).rejects.toThrow();
    });
    test("sqrt", () => {
        ...
        expect(runFunc(functions.sqrt, ['r0', -4],)).rejects.toThrow();
    ...
    test("rand", () => {
        expect(runFunc(functions.rand, ['r0', NaN],)).rejects.toThrow()
        expect(runFunc(functions.rand, ['r0', 0])).resolves.toBeGreaterThanOrEqual(0);
        expect(runFunc(functions.rand, ['r0', 0])).resolves.toBeLessThanOrEqual(1);
    });
    test("sll", () => {
